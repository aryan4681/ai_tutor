...
def get_topic_suggestions(subject, feedback_list, questions_list, overall_percentage, mode_type, current_topic=None):
    ...
    if mode_type == 'comprehensive':
        ...
        prompt = (
             f"A student took a comprehensive exam on the subject '{subject}'. "
             f"They answered the following questions incorrectly:\n"
             f"{'\n'.join(incorrect_questions_details)}\n\n"
             f"Based ONLY on these incorrect answers, which 1-3 specific topics from the following list seem to be the weakest areas for the student? "
             f"List only the topic names, separated by commas. Do not add any other text, explanations, or introductory phrases like 'Based on the incorrect answers...' or 'The weak areas seem to be...'. "
             f"If you cannot reliably determine specific weak topics from the questions provided, respond ONLY with the single word 'None'.\n\n"
             f"Available topics for {subject}: { ', '.join(subject_topics)}"
        )
